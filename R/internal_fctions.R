                     ...
                     base_url = "https://api.webstat.banque-france.fr/webstat-") 
{
  ...
  opt_params <- c(series_name, startPeriod, endPeriod, firstNObs, 
                  lastNObs)
  opt_txt <- c(NA, "startPeriod=", "endPeriod=", "firstNObservations=", 
               "lastNObservations=")
...
get_data <- function(url, client_ID, ...) 
{
  ...
  if(req$status_code != 200) {
    switch(as.character(req$status_code),
      ...
      "429" = {stop(paste(content(req,encoding = "UTF-8")$httpMessage,"\n",content(req,encoding = "UTF-8")$moreInformation))},
...
parse_json_unit <- function(main_content,dataset_name) { 
  ...
  attr(df, "metadata") <- lapply(attr(df,"metadata"),function(x) if(is.null(x)) return(NA) else return(x))
...
parse_json <- function(req) {
  ...
  dates_list <- lapply(df,function(x) return(x$date))
...
parse_csv <- function(req, language) {
  ...
  meta_data_df <- readr::read_delim(file = cont_csv, delim = sep_lg, 
                                    skip = 0, col_names = FALSE, n_max = 3)
  
  data_serie <- readr::read_delim(cont_csv, delim=sep_lg, na = "-",
                                  col_names = FALSE, 
